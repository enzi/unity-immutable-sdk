#if (!IMMUTABLE_CUSTOM_BROWSER || UNITY_STANDALONE_LINUX) && (UNITY_STANDALONE_WIN || UNITY_STANDALONE_LINUX || (UNITY_ANDROID && UNITY_EDITOR_WIN) || (UNITY_IPHONE && UNITY_EDITOR_WIN))
...
namespace VoltstroStudios.UnityWebBrowser
{
    public class UwbWebView : MonoBehaviour, IWebBrowserClient
    {
        ...
        public async UniTask Init(int engineStartupTimeoutMs, bool redactTokensInLogs, Func<string, string> redactionHandler)
        {
            ...
            // Logger
            webBrowserClient.Logger = new DefaultUnityWebBrowserLogger(); //new DefaultUnityWebBrowserLogger(redactionHandler: redactTokensInLogs ? redactionHandler : null);
            ...
            browser.inputHandler = new WebBrowserInputSystemHandler()
            {
                
            };// ScriptableObject.CreateInstance<WebBrowserInputHandler>();
